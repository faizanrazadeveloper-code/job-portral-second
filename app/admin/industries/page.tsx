...
import { Layers, ShieldCheck, Briefcase } from "lucide-react";
import { industries as d } from "@/lib/admin-data";

export const metadata = { title: "Industries · Energy Tail Admin" };

export default function IndustriesPage() {
  return (
    <div className="flex min-h-screen bg-slate-50">
      <AdminSidebar active="industries" />

      <div className="flex-1 min-w-0 flex flex-col">
        <AdminTopbar variant="dark" />

        <main className="flex-1 p-4 sm:p-6 space-y-6">
          <AdminPageHeader crumbs={["Dashboard", "Industries"]} title="Industries" addLabel="Add New Industry" />

          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <AdminStatCard icon={Layers} tone="blue" label="Total Industries" value={d.stats.total} hint="All industries" />
            <AdminStatCard icon={ShieldCheck} tone="green" label="Active Industries" value={d.stats.active} hint="Currently active" />
            <AdminStatCard icon={Layers} tone="orange" label="Inactive Industries" value={d.stats.inactive} hint="Currently inactive" />
            <AdminStatCard icon={Briefcase} tone="purple" label="Total Jobs" value={d.stats.jobs.toLocaleString()} hint="Under all industries" />
          </div>

          <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6 items-start">
            <div className="min-w-0 bg-white rounded-2xl border border-slate-100 overflow-hidden">
              <AdminToolbar searchPlaceholder="Search industries..." />
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-t border-b border-slate-100 text-[12px] font-semibold text-slate-400 uppercase tracking-wide">
                      ...
                      <th className="px-2 py-3">Industry Name</th>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-slate-100 p-5">
                <div className="text-[14px] font-bold text-slate-900 mb-4">Industry Overview</div>
                <AdminDonutChart data={d.donut} total={d.stats.total} />
              </div>
              <AdminTopList title="Top Industries by Jobs" items={d.top} footerLabel="View all industries" />
